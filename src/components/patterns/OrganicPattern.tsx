...
uniform vec3 uLightDColor;
uniform vec3 uLightDPosition;
uniform float uLightDIntensity;
uniform vec3 uLightEColor;
uniform vec3 uLightEPosition;
uniform float uLightEIntensity;
...
void main()
{
    ...
    float lightDIntensity = max(0.0, - dot(computedNormal.xyz, normalize(- uLightDPosition))) * uLightDIntensity;
    float lightEIntensity = max(0.0, - dot(computedNormal.xyz, normalize(- uLightEPosition))) * uLightEIntensity;
    ...
    color = mix(color, uLightDColor, lightDIntensity * fresnel);
    color = mix(color, uLightEColor, lightEIntensity * fresnel);
...
const OrganicPattern: React.FC<OrganicPatternProps> = ({ audioData, dimensions, config, showControls }) => {
    ...
    const animationStateRef = useRef({ audioData, config });

    useEffect(() => {
        animationStateRef.current = { audioData, config };
    });
    ...
    useEffect(() => {
        ...
        // Camera
        const camera = new THREE.PerspectiveCamera(25, dimensions.width / dimensions.height, 0.1, 15);
        ...
        const bloomPass = new UnrealBloomPass(new THREE.Vector2(dimensions.width, dimensions.height), 0.1, 0.1, 0);
        ...
        const material = new THREE.ShaderMaterial({
            uniforms: {
                ...
                uLightAIntensity: { value: 10.85 },
                uLightBColor: { value: new THREE.Color('#0063ff') },
                uLightBPosition: { value: new THREE.Vector3(-1, -1, 0) },
                uLightBIntensity: { value: 20.4 },
                uLightCColor: { value: new THREE.Color('#ff00ff') },
                uLightCPosition: { value: new THREE.Vector3(0, 1, 1) },
                uLightCIntensity: { value: 1.2 },
                uLightDColor: { value: new THREE.Color('#00ffff') },
                uLightDPosition: { value: new THREE.Vector3(1, 0, -1) },
                uLightDIntensity: { value: 5.2 },
                uLightEColor: { value: new THREE.Color('#ffff00') },
                uLightEPosition: { value: new THREE.Vector3(-1, 0, 1) },
                uLightEIntensity: { value: 1.2 },
                ...
                uDisplacementFrequency: { value: 4.120 },
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            const { audioData: currentAudioData, config: currentConfig } = animationStateRef.current;

            if (materialRef.current) {
                materialRef.current.uniforms.uTime.value = elapsedTime * currentConfig.motionIntensity;
                materialRef.current.uniforms.uDisplacementStrength.value = currentAudioData.volume * 3.0 * currentConfig.sensitivity;
                materialRef.current.uniforms.uDistortionStrength.value = currentAudioData.bassEnergy * 1.5 * currentConfig.sensitivity;
                materialRef.current.uniforms.uFresnelMultiplier.value = 2.587 + (currentAudioData.midEnergy * 2 * currentConfig.sensitivity);
            }
    ...
    return <div ref={mountRef} className='absolute inset-0'>
        <CreditsComponent href="https://x.com/bruno_simon/status/1448656953496932358" label="Adapted from @bruno_simon" show={showControls} />
    </div>;
};
